...
// Database setup
const db = new sqlite3.Database('communication_ltd.db');
...
const escapeHtmlFast = (unsafe) => {
  if (typeof unsafe !== 'string') return unsafe;
  return unsafe.replace(
    /[&<>"']/g,
    (match) => htmlEscapeMap.get(match) || match
  );
};

// Efficiently escape multiple fields at once
const escapeObjectFields = (obj, fieldsToEscape) => {
  const escaped = { ...obj };
  fieldsToEscape.forEach((field) => {
    if (escaped[field] && typeof escaped[field] === 'string') {
      escaped[field] = escapeHtmlFast(escaped[field]);
    }
  });
  return escaped;
};

// Middleware for automatic XSS protection on specific routes
const xssProtection = (fieldsToEscape) => {
  return (req, res, next) => {
    if (req.body && typeof req.body === 'object') {
      req.body = escapeObjectFields(req.body, fieldsToEscape);
    }
    next();
  };
};
...
app.post('/api/login', (req, res) => {
  ...
  db.get(
    ...
    async (err, user) => {
      ...
      // Verify password using HMAC + Salt
...
app.post(
  ...
  (req, res) => {
    ...
    // Data is already escaped by middleware, so we can use it directly
    // User input is safely passed as parameters (?) instead of being concatenated into the SQL string.
    db.run(
      'INSERT INTO clients (fullName, email, phone, packageName, sector, address) VALUES (?, ?, ?, ?, ?, ?)',
      [fullName, email, phone, packageName, sector, address],
      function (err) {
        if (err) {
